...
const Day2 = () => {
  ...
  const renderTable = (date, day) => (
    <table className="schedule-table">
      <thead>
        <tr className="top-date">
          <th colSpan="25">{`${date} - ${day}`}</th>
        </tr>
        <tr>
          <th className="header">Time</th>
          {[...Array(24).keys()].map((hour) => (
            <th key={hour} className={`time-slot ${hour % 2 === 0 ? "" : ""}`}>
                ...
                backgroundColor:
                  [0, 2, 4, 6, 20, 22].includes(hour)
                    ...
                    : hour >= 8 && hour <= 18
                      ...
                      : "transparent",
                color:
                  hour >= 8 && hour <= 18 || [0, 2, 4, 6, 20, 22].includes(hour)
                    ? "white"
                    : "inherit",
                textAlign:
                  hour >= 8 && hour <= 18 || [0, 2, 4, 6, 20, 22].includes(hour)
  ...
  return (
    <div className="schedule-container">
      ...
      <div className="top-year">
        <div colSpan="25">1st Oct 2022 - 2nd Oct 2022</div>
      </div>

      <div className="schedule-tables">
        <div className="table-wrapper">
          {renderTable("Saturday", "1 October")}
        </div>
        <div className="table-wrapper">
          {renderTable("Sunday", "2 October")}
        </div>
      </div>
    </div>
  );
